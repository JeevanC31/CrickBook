...
interface Booking {
  id: string;
  category: 'Turf' | 'Coach' | 'Sports Items' | 'Match';
  title: string;
  subtitle: string;
  startTime: string;
  endTime: string;
      ...
      {bookings.length === 0 ? (
        ...
        <div className="orders-grid">
          {bookings.map((booking, i) => {
            ...
            if (isMatch) iconBg = { background: 'linear-gradient(135deg, #0ea5e9, #22c55e)' };
                ...
                <div className="order-card-header">
                  ...
                  <div className="order-card-title">
                    ...
                    <span className="order-location">
                      <i className={`fa-solid ${isCoach ? 'fa-star' : isShop ? 'fa-money-bill' : isMatch ? 'fa-stadium' : 'fa-location-dot'}`}></i>
                      {booking.subtitle}
                    </span>
                  </div>
                ...
                <div className="order-card-details">
                  ...
                  {!isShop ? (
                    <>
                      ...
                      <div className="order-detail">
                        <i className={`fa-solid ${isMatch ? 'fa-user-ninja' : 'fa-users'}`}></i>
                        <div>
                          <span className="detail-label">{isMatch ? 'Role' : 'Players'}</span>
                          <span className="detail-value">
                            {isMatch ? booking.fullItemsList : `${booking.guests || 1} Person${booking.guests !== 1 ? 's' : ''}`}
                          </span>
                        </div>
                      </div>
                    </>
