...
const ProductCard = ({ product, cartItem, onAdd, onUpdate }: {
  product: Product;
  ...
  let imgs: string[] = [];
  try { imgs = JSON.parse(product.imageUrls || '[]'); } catch {}
  if (!imgs.length && product.imageUrl) imgs = [product.imageUrl];

  const [imgIdx, setImgIdx] = React.useState(0);

  return (
    <div className="coach-card glass" style={{ padding: '1rem' }}>
      <div style={{ height: '160px', borderRadius: '8px', overflow: 'hidden', marginBottom: '1rem', position: 'relative', background: 'rgba(255,255,255,0.05)' }}>
        {imgs.length > 0 ? (
          <>
            ...
            {imgs.length > 1 && (
              <>
                <button onClick={() => setImgIdx(i => (i - 1 + imgs.length) % imgs.length)}
                  style={{ position: 'absolute', left: 4, top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.5)', border: 'none', color: '#fff', width: 24, height: 24, borderRadius: '50%', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>‹</button>
                <button onClick={() => setImgIdx(i => (i + 1) % imgs.length)}
                  style={{ position: 'absolute', right: 4, top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.5)', border: 'none', color: '#fff', width: 24, height: 24, borderRadius: '50%', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>›</button>
                <div style={{ position: 'absolute', bottom: 6, left: 0, right: 0, display: 'flex', justifyContent: 'center', gap: '4px' }}>
                  {imgs.map((_, i) => <span key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: i === imgIdx ? '#fff' : 'rgba(255,255,255,0.4)' }}></span>)}
                </div>
              </>
            )}
          </>
      ...
      <div style={{ flex: 1 }}>
        ...
        {cartItem ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '1rem' }}>
            <button className="btn btn-outline" style={{ padding: '0.2rem 0.6rem' }} onClick={() => onUpdate(product.id, -1)}>-</button>
            <span>{cartItem.cartQuantity}</span>
            <button className="btn btn-outline" style={{ padding: '0.2rem 0.6rem' }} onClick={() => onUpdate(product.id, 1)}>+</button>
          </div>
        ) : (
          <button className="btn btn-primary w-100 mt-3" onClick={() => onAdd(product)} disabled={product.stock === 0}>
...
const Shop = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const res = await fetch(`${SHOP_URL}/shop`);
        if (res.ok) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      const SHOP_URL = process.env.NEXT_PUBLIC_SHOP_URL || '';
      const res = await fetch(`${SHOP_URL}/cart/checkout`, {
        method: 'POST',
